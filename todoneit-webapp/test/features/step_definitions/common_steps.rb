...
Given /I have no data/ do
  $browser.goto host + '/admin/destroyall'
end
...
Then /^the field "([^\"]*)" contains "([^\"]*)"$/ do |name, contents|
    fail if not $browser.text_field(:name => name).value == contents
end

  Then /I should see the text "(.*)"/ do |text|
    fail if not $browser.contains_text(text)
  end
